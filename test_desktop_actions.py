...
from spick_folio.desktop_actions import (
...
class DesktopActionTests(unittest.TestCase):
    @patch('spick_folio.desktop_actions.shutil.which', return_value='/usr/bin/xdg-open')
    @patch('spick_folio.desktop_actions.subprocess.Popen')
    @patch('spick_folio.desktop_actions.validate_open_path')
    ...
    def test_open_path_on_desktop_opens_file(self, mock_validate, mock_popen, _which):
        ...
        mock_proc.wait.return_value = 0
        mock_popen.return_value = mock_proc
        with patch('spick_folio.desktop_actions.os.path.isfile', return_value=True):
    ...
    @patch('spick_folio.desktop_actions.shutil.which', return_value='/usr/bin/notify-send')
    @patch('spick_folio.desktop_actions.subprocess.run')
